...
class LoadDimensionOperator(BaseOperator):
    ...
    def execute(self, context):
        """
        - Loads Dimension table from Redshift
        - Appends or deletes-inserts data from Staging tables
        """
        redshift = PostgresHook(postgres_conn_id=self.redshift_conn_id)
        
        # Load Dimension table from Redshift
        self.log.info('Loading Dimension table %s from Redshift' % self.table_name)
        ...
        self.log.info('Finished loading Dimension table %s' % self.table_name)
